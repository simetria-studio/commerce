...
@section('container')
                    ...
                    <div class="card card-primary card-outline">
                        ...
                        <div class="card-body pad table-responsive">
                            <div class="container">
                                <div class="row">
                                    <div class="col-6">
                                        <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#novoAfiliado"><i class="fas fa-plus"></i> Novo Afiliado</button>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        @forelse ($accounts as $account)
                                            <tr class="tr-id-{{$account->id}}">
                                                ...
                                                <td>{{$account->email}}</td>
                                                <td>R$ {{number_format($account->bank->balance_available, 2, ',', '.')}} <button type="button" class="btn btn-info btn-sm btn-editar" data-toggle="modal" data-target="#pagarAfiliado" data-dados="{{json_encode($account)}}"><i class="fas fa-dollar-sign"></i></button></td>
                                                ...
                                                <td>
                                                    <div class="btn-group" role="group" aria-label="">
                                                        ...
                                                        <a href="#" class="btn btn-info btn-sm btn-editar" data-toggle="modal" data-target="#editarCliente" data-dados="{{json_encode($account)}}"><i class="fas fa-edit"></i> Editar Afiliado</a>
    ...
    <div class="modal fade" id="novoAfiliado">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="#" method="post" id="postNovoAfiliado">
                    @csrf
                    ...
                    <div class="modal-footer justify-content-between">
                        ...
                        <button type="button" class="btn btn-success btn-salvar" data-update_table="S" data-save_target="#postNovoAfiliado" data-save_route="{{route('novoAfiliado')}}"><i class="fas fa-save"></i> Salvar</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="pagarAfiliado">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="#" method="post" id="postPagarAfiliado">
                    @csrf
                    <input type="hidden" name="id">
                    <div class="modal-header">
                        <h4 class="modal-title">Pagar Afiliado - <span class="_name"></span></h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    ...
                    <div class="modal-footer justify-content-between">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fas fa-times"></i> Fechar</button>
                        <button type="button" class="btn btn-success btn-salvar" data-save_target="#postPagarAfiliado" data-save_route="{{route('pagarAfiliado')}}"><i class="fas fa-save"></i> Salvar</button>
                    </div>
                </form>
            </div>
        </div>
